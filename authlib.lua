if game.CoreGui:FindFirstChild("EdinsHub") then
    game.CoreGui.EdinsHub:Destroy()
end
game:GetService("UserInputService").InputBegan:connect(
    function(key, gpe)
        if key.KeyCode == Enum.KeyCode.RightControl then
            pcall(
                function()
                    for i, v in pairs(game.CoreGui.EdinsHubLib:GetChildren()) do
...
local edinLib = {RainbowColorValue = 0, HueSelectionPosition = 0}
...
coroutine.wrap(
	function()
		while wait() do
			edinLib.RainbowColorValue = edinLib.RainbowColorValue + 1 / 255
			edinLib.HueSelectionPosition = edinLib.HueSelectionPosition + 1

			if edinLib.RainbowColorValue >= 1 then
				edinLib.RainbowColorValue = 0
			end

			if edinLib.HueSelectionPosition == 93 then
				edinLib.HueSelectionPosition = 0
			end
		end
	end
)()
...
local EdinsHubLib = Instance.new("ScreenGui")
EdinsHubLib.Name = "EdinsHub"
EdinsHubLib.Parent = game.CoreGui
EdinsHubLib.ZIndexBehavior = Enum.ZIndexBehavior.Sibling

function edinLib:Window()
				...
				function()
					...
					if RainbowColorPicker then
						...
						while RainbowColorPicker do
							CurrentColor.BackgroundColor3 = Color3.fromHSV(edinLib.RainbowColorValue, 1, 1)
							Color.BackgroundColor3 = Color3.fromHSV(edinLib.RainbowColorValue, 1, 1)

							ColorSelection.Position = UDim2.new(1, 0, 0, 0)
							HueSelection.Position = UDim2.new(0.48, 0, 0, edinLib.HueSelectionPosition)
